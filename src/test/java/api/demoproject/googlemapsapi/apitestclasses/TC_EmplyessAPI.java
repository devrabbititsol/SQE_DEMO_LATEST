package api.demoproject.googlemapsapi.apitestclasses;
...
import api.demoproject.googlemapsapi.apipageclasses.APU_TestDataset;
public class TC_EmplyessAPI extends BaseClass {
	...
	public TC_EmplyessAPI() throws Exception {
		PropertyConfigurator.configure("log4j.properties");
		logger = Logger.getLogger(TC_EmplyessAPI.class);
	...
	public void doRequest() throws Exception { 
		...
		String primaryInfo = APU_TestDataset.primaryInfo;
		Constants.PRIMARY_INFO = primaryInfo;
		reports = ExtentConfigurations.getExtentInstance(reportsPath, projectPath ,APU_TestDataset.projectName);
		int datasetLength = APU_TestDataset.datasetLength; 
		for(int i = 1; i <= datasetLength; i++) {
		test = reports.startTest("TC_EmplyessAPI - " + datasetNames[i-1]);

			boolean isExecutionFlag = true;			
		/*--------------------- GET --------------------- */

			if(isExecutionFlag) {
 			 	Object object = (Object) new APU_TestDataset();
